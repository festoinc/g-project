# MCP servers with the G-PROJECT

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the G-PROJECT.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the G-PROJECT through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the Gemini model and your local environment or other services like APIs.

An MCP server enables the G-PROJECT to:
...
- **Access resources:** Read data from specific resources (though the G-PROJECT primarily focuses on tool execution).

With an MCP server, you can extend the G-PROJECT's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The G-PROJECT integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
The G-PROJECT supports three MCP transport types:
...
The G-PROJECT uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
When the G-PROJECT starts, it performs MCP server discovery through the following detailed process:
...
This comprehensive integration makes MCP servers a powerful way to extend the G-PROJECT's capabilities while maintaining security, reliability, and ease of use.
