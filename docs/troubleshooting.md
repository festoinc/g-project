...
- **Q: How do I update G-PROJECT to the latest version?**
  - A: If installed globally via npm, update G-PROJECT using the command `npm install -g @festoinc/g-project@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are G-PROJECT configuration files stored?**
...
- **Error: Command not found (when attempting to run G-PROJECT).**
  - **Cause:** G-PROJECT is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure G-PROJECT installation was successful.
