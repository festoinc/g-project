...
This monorepo contains two main packages: `@festoinc/g-project` and `@festoinc/g-project-core`.

## `@festoinc/g-project`

This is the main package for the G-PROJECT. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@festoinc/g-project-core`. This means that whether a user installs the package with `npm install -g @festoinc/g-project` or runs it directly with `npx @festoinc/g-project`, they are using this single, self-contained executable.

## `@festoinc/g-project-core`
...
Releases are managed through the [release.yml](https://github.com/festoinc/g-project/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
Every night at midnight UTC, the [Release workflow](https://github.com/festoinc/g-project/actions/workflows/release.yml) runs automatically on a schedule. It performs the following steps:
...
```bash
npm install -g @festoinc/g-project@nightly
```
...
After the workflow has successfully completed, you can monitor its progress in the [GitHub Actions tab](https://github.com/festoinc/g-project/actions/workflows/release.yml). Once complete, you should:

1.  Go to the [pull requests page](https://github.com/festoinc/g-project/pulls) of the repository.
...
- `npx -y @festoinc/g-project@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @festoinc/g-project@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @festoinc/g-project && npm uninstall -g @festoinc/g-project && npm cache clean --force &&  npm install @festoinc/g-project@<version>`
...
1.  Go to the [Actions tab](https://github.com/festoinc/g-project/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/google-g-project-0.1.6.tgz`).
...
1.  The `package.json` is Transformed:
    ...
    - Why: The final package.json must be different from the one used in development. Key changes include:
      ...
      - Removing workspace-specific "dependencies": { "@g-project/core": "workspace:\*" } and ensuring the core code is
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @festoinc/g-project`.
